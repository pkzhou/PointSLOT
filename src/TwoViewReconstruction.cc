...
bool TwoViewReconstruction::Reconstruct(const std::vector<cv::KeyPoint>& vKeys1, const std::vector<cv::KeyPoint>& vKeys2, const vector<int> &vMatches12,
                                        std::vector<bool> &vbMatchesInliers, cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    mvKeys1.clear();
    mvKeys2.clear();
    ...
    if(RH>0.50) // if(RH>0.40)
    {
        ...
        bFlag = ReconstructH(vbMatchesInliersH,H, mK,R21,t21,vP3D,vbTriangulated,minParallax,50);
        vbMatchesInliers = vbMatchesInliersH;
    }
    else //if(pF_HF>0.6)
    {
        //cout << "Initialization from Fundamental" << endl;
        bFlag =  ReconstructF(vbMatchesInliersF,F,mK,R21,t21,vP3D,vbTriangulated,minParallax,50);
        vbMatchesInliers = vbMatchesInliersF;
    }
    return bFlag;
}
